...
export async function createWallet(opts: CreateWalletOptions): Promise<WalletContext> {
  ...
  const wallet = await WalletFacade.init({
    ...
    shielded: async (config: unknown) => {
      const cls = ShieldedWallet(config as any);
    ...
    unshielded: async (config: unknown) => {
      const cls = UnshieldedWallet(config as any);
    ...
    dust: async (config: unknown) => {
      const cls = DustWallet(config as any);
...
export async function persistWalletState(
  ...
  for (const kind of CHILD_KINDS) {
    try {
      const child = (ctx.wallet as Record<ChildKind, { serializeState: () => Promise<unknown> }>)[kind];
